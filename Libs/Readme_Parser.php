...
if (!defined('WORDPRESS_README_MARKDOWN')) {
    if (defined('AUTOMATTIC_README_MARKDOWN')) {
        ...
        define('WORDPRESS_README_MARKDOWN', JLTMB_PATH . 'Libs/Markdown.php');
    }
}
...
class Readme_Parser
{
    ...
    function parse_readme_contents($file_contents)
    {
        ...
        foreach (['tags', 'contributors', 'requires_at_least', 'tested_up_to', 'stable_tag', 'donate_link'] as $chop) {
            if ($$chop) {
                $_chop         = '_' . $chop;
        ...
        for ($i = 1; $i <= count($_sections); $i += 2) {
            $_sections[$i] = preg_replace('/^[\s]*=[\s]+(.+?)[\s]+=/m', '<h4>= $1 =</h4>', isset($_sections[$i]) ? $_sections[$i] : '');
        ...
        $final_sections = [];
        foreach (['description', 'installation', 'frequently_asked_questions', 'screenshots', 'changelog', 'change_log', 'upgrade_notice'] as $special_section) {
        ...
        if (!isset($final_sections['description'])) {
            $final_sections = array_merge(['description' => $this->filter_text($_short_description[2], true)], $final_sections);
            $excerpt        = true;
        }
    ...
    function filter_text($text, $markdown = false)
    {
        ...
        $text = call_user_func([__CLASS__, 'code_trick'], $text, $markdown); // A better parser than Markdown's for: backticks -> CODE
    ...
    function code_trick($text, $markdown)
    {
        ...
        if ($markdown) {
            $text = preg_replace_callback('!(<pre><code>|<code>)(.*?)(</code></pre>|</code>)!s', [__CLASS__, 'decodeit'], $text);
        }
