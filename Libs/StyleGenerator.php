...
if ( ! class_exists( 'MbStyleGenerator' ) ) {
	class MbStyleGenerator {
		...
		public function enqueue_frontend_styles() {
			...
			if ( ! empty( $post ) && ! empty( $post->ID ) ) {
				...
				if ( file_exists( $upload_dir['basedir'] . '/mb-style/mb-style-' . abs( $post->ID ) . '.min.css' ) ) {
                    $file_path = $upload_dir['baseurl'] . '/mb-style/mb-style-' . $post->ID . '.min.css';
					wp_enqueue_style( 'mb-block-style-' . $post->ID, $file_path, [], md5_file( $file_path ) );
				}
			}
		}
		...
		private static function write_block_styles( $block_styles, $post ) {
			...
			if ( ! empty( $css = self::build_css( $block_styles ) ) ) {
				$upload_dir = wp_upload_dir()['basedir'] . '/mb-style/';
				if ( ! file_exists( $upload_dir ) ) {
					mkdir( $upload_dir );
				}
				file_put_contents( $upload_dir . 'mb-style-' . abs( $post->ID ) . '.min.css', $css );
			}
		}
