...
use JLTMB\Inc\Classes\Notifications\Base\Date;
...
class Pro_Upgrade {
	...
	public function __construct() {
		$this->slug = Helper::jltmb_slug_cleanup();
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltmb_dashboard_widget',                          // Widget slug.
			esc_html__('Master Blocks News & Updates', 'ultimate-blocks-for-gutenberg' ), // Title.
		...
		$example_widget_backup = array( 'jltmb_dashboard_widget' => $default_dashboard['jltmb_dashboard_widget'] );
		unset( $default_dashboard['jltmb_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltmb-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltmb-dashboard-promo" style="--jltmb-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="ultimate-blocks-for-gutenberg Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltmb-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'ultimate-blocks-for-gutenberg' );
		...
		<div class="jltmb-dashboard_footer">
			<ul>
				<li class="jltmb-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'ultimate-blocks-for-gutenberg' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="jltmb-overview__help">
                    <a href="https://jeweltheme.com/docs/master-blocks" target="_blank">
                        Help
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'ultimate-blocks-for-gutenberg' ); ?>
                        </span>
                ...
                <li class="jltmb-overview__upgrade">
                    <a href="https://jeweltheme.com/master-blocks/pricing" target="_blank">
                        Upgrade
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'ultimate-blocks-for-gutenberg' ); ?>
                        </span>
		...
		<style>
			...
			.jltmb-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltmb-rss-widget .jltmb-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltmb-rss-widget .jltmb-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltmb-rss-widget .jltmb-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltmb-rss-widget .jltmb-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltmb-rss-widget .jltmb-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltmb_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltmb_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltmb_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTMB_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltmb_sheet_promo_data', $remote_data );
			update_option( 'jltmb_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltmb_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltmb_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltmb-popup jltmb-upgrade-popup" id="jltmb-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltmb-popup-overlay"></div>

			<div class="jltmb-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltmb-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">

				<!-- close  -->
				<div class="jltmb-popup-modal-close popup-dismiss">×</div>

				<!-- content section  -->
				<div class="jltmb-popup-modal-footer">

					<!-- countdown  -->
					<div class="jltmb-popup-countdown" style="display: none;">
						<span class="jltmb-popup-countdown-text">Deal Ends In</span>
						<div class="jltmb-popup-countdown-time">
							<div>
					...
					<!-- button  -->
					<a class="jltmb-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltmb-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltmb-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltmb-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltmb-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltmb-popup-countdown").hide(0);
				}
